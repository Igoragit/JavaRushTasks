...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        if(deep>=1){
            for (int x=0; x<humansRelationships.length; x++){
                if(x<index && humansRelationships[index][x]){
                    set.add(x);
                    set.addAll(getAllFriendsAndPotentialFriends(x,deep-1));
                } else if(x>index && humansRelationships[x][index]){
                    set.add(x);
                    set.addAll(getAllFriendsAndPotentialFriends(x,deep-1));
                }
            }
